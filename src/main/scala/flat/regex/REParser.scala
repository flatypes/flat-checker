...
object REParser extends Parsers:
  ...
  private def allChar: Parser[RegExpr] = '.' ^^^ RegExpr.allChar
  ...
  private def charSet: Parser[RegExpr] =
    for
      _ <- '['
    ...
    yield RegExpr.REChar(if neg then !cs else cs)

  private def base: Parser[RegExpr] =
    allChar | (not(".^$*+?\\[]|(){") | escapeSeq) ^^ RegExpr.fromChar | charSet | '(' ~> expr <~ ')'

  private def fin: Parser[Int] = rep1("0123456789") ^^ (cs => cs.mkString.toInt)

  private def loopQuantifier: Parser[(Int, Option[Int])] =
    for
      in <- getInput
      lb <- '{' ~> (fin | success(0))
      ub <- ',' ~> (fin ^^ Some.apply | success(None)) <~ '}'
    yield (lb, ub)

  private def repeat: Parser[RegExpr] =
    base <~ '*' ^^ { r => r.* } | base <~ '+' ^^ { r => r.+ } | base <~ '?' ^^ { r => r.? } |
      base ~ ('{' ~> fin <~ '}') ^^ { case r ~ k => r ^ k } |
      base ~ loopQuantifier ^^ { case r ~ (l, u) => RegExpr.mkLoop(l, u, r) } | base

  private def concat: Parser[RegExpr] = (repeat | '}' ^^ RegExpr.fromChar).* ^^ RegExpr.mkConcat

  private def expr: Parser[RegExpr] = repsep(concat, '|') ^^ RegExpr.mkUnion
  ...
  def tryParse(input: CharSequence): Either[String, RegExpr] =
  ...
  def parse(input: CharSequence): RegExpr = tryParse(input) match
