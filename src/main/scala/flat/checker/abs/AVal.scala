...
type AVal = ATop.type | AInt | ABool | AString | AFun
...
object AVal:
  ...
  private def computeUb(typ: Type): AVal =
    typ match
      ...
      case IntType => AInt.fromRange(Range.full)
      case BoolType => ABool.Top
      case CharType => ???
      case StringType => ReLang.full
      case LiteralType(n: Int) => AInt.fromRange(Range.fromInt(n))
      case LiteralType(b: Boolean) => ABool.fromBoolean(b)
      case LiteralType(c: Char) => ???
      ...
      case FunType(ts, t) =>
        ...
        val u = computeUb(t)
        AFun(us, u)
      case RangeType(lb, ub) => AInt.fromRange(Range(lb, ub))
...
extension (v: AVal)
  def show: String = v match
    case ATop => "⊤"
    case AInt(_, Some(_, k)) => s"index($k)"
    case AInt(r, _) => if r == Range.full then "Int" else r.toString
