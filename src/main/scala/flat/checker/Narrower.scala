...
class Narrower(using config: Config) extends LazyLogging:
  ...
  private def narrowBy(hypothesis: Expr, res: Map[Expr, Domain])(using ctx: PrfCtx): Map[Expr, Domain] =
    hypothesis match
      ...
      case Cmp(op@(EQ | NE), CharAt(es@Var(_), ei), Const(t: String)) if t.length == 1 =>
        ...
        yield Substring(es, base, StringLength(es)) -> r.narrowByChatAt(k, cs)
        if p.isDefined then
          return res + p.get
        // Ordinary
      ...
      // narrow AList
      case StringContains(SeqGet(e, ei), Const(t: String)) if t.length == 1 =>
        val c = t.head
        getList(e, res) match
          case Some(list) =>
            val indexInferer = new IndexInferer
            ...
            res + (e -> list1)
          case None => res
      case Not(StringContains(SeqGet(e, ei), Const(t: String))) if t.length == 1 =>
        val c = t.head
        getList(e, res) match
          case Some(list) =>
            val indexInferer = new IndexInferer
      ...
      case Cmp(op@(EQ | NE), SeqGet(e, ei), Const(t: String)) =>
        getList(e, res) match
          case Some(list) =>
            ...
            res + (e -> list1)
          case None => res
      case Not(SeqContains(SeqSlice(e, ei, ej), Const(t: String))) =>
        getList(e, res) match
          case Some(list) =>
            val indexInferer = new IndexInferer
            val list1 =
  ...
  private def getList(lst: Expr, res: Map[Expr, Domain])(using ctx: PrfCtx): Option[AList] =
    res.get(lst) match
      case Some(l: AList) => Some(l)
      case _ =>
        val ctx1 = ctx ++ List.from(res.collect { case (e, l: AList) => RefinedBy(e, l) })
