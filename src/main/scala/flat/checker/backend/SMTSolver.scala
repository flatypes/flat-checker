...
import flat.checker.backend.SolverResult.*
...
object SMTSolver:
  ...
  def prove(goal: Expr, premises: List[Expr])(using types: Types): SolverResult =
    ...
    if result.isUnsat then Valid
      ...
      val terms = Array.from(for x <- goal.collectVars yield ctx(x))
  ...
  private def encodeType(typ: Type): Sort = typ.toSort match
    ...
    case checker.Sort.Int => smt.getIntegerSort
    case checker.Sort.Bool => smt.getBooleanSort
    case checker.Sort.String => smt.getStringSort
    case checker.Sort.Tuple(ts) => smt.mkTupleSort(ts.map(encodeType(_)).toArray)
    case checker.Sort.Array(t) => smt.mkArraySort(smt.getIntegerSort, encodeType(t))
  ...
  private object Encoder extends ExprVisitor[Ctx, Term]:
    ...
    override def visitCharToCode(node: CharToCode, ctx: Ctx): Term =
      val s = node.char.accept(this, ctx)
      // If the string does not have length one, return -1.
      smt.mkTerm(Kind.STRING_TO_CODE, s)

    override def visitCharFromCode(node: CharFromCode, ctx: Ctx): Term =
