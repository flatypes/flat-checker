...
val strMemberTable = Map(
  ...
  "__add__" -> MemberInfo(Seq(Sort.String), Seq(), Sort.String, { case Seq(s1, s2) => StrConcat(s1, s2) }),
  "__len__" -> MemberInfo(Seq(), Seq(), Sort.Int, { case Seq(s) => StrLen(s) }),
  ...
  "__getitem_slice__" -> MemberInfo(Seq(Sort.Int), Seq(Sort.Int -> Const(-1)), Sort.String,
