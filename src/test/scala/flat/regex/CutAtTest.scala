...
class CutAtTest extends RegexTest:
  ...
  import REOps.*
  import RegExpr.RENull

  test("cut abc"):
    val r = re("abc")
    assert(cutAt(r, 0) == List((RENull, ch('a'), re("bc"))))
    assert(cutAt(r, 1) == List((re("a"), ch('b'), re("c"))))
    assert(cutAt(r, 2) == List((re("ab"), ch('c'), RENull)))
    assert(cutAt(r, 3) == Nil)
