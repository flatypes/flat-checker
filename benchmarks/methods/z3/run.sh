...
# NOTE: this is intended to be run from within the z3 docker container
# see README for more information

TIMEOUT_SECONDS=60

subjects_dir="/benchmarks/$1/smt"
results_dir="/benchmarks/methods/z3/results-$1"
...
find "$subjects_dir" -mindepth 2 -maxdepth 2 -type f -name "*.smt2" | sort -n | while read -r filepath; do
  ...
  z3 -smt2 -T:$TIMEOUT_SECONDS "$filepath" > $outfile 2>&1
  exit_code=$?
  end_time=$(date +%s%3N)
