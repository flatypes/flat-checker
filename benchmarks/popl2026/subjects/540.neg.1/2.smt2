...
(assert (let ((_let_1 (str.to_re "b"))) (let ((_let_2 (str.to_re "a"))) (let ((_let_3 (re.* (re.diff re.allchar _let_2)))) (let ((_let_4 (re.++ (re.++ _let_1 _let_3) _let_2))) (not (str.in_re (str.substr s 0 (- (str.len s) 0)) (re.++ (re.++ (re.++ _let_3 _let_2) (re.opt (re.++ (re.++ _let_1 (re.* (re.++ _let_4 _let_1))) (re.opt _let_4)))) (re.* (re.++ (re.diff re.allchar _let_1) (re.* _let_1)))))))))))
(check-sat)
(exit)